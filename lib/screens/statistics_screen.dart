...
class _StatisticsScreenState extends State<StatisticsScreen> {
  ...
  Widget _buildOverallStats(List<CourseProgress> progressList) {
    ...
    final completedLessons = progressList.fold<int>(
      0,
      (sum, progress) =>
          sum + progress.lessonProgresses.where((l) => l.completed).length,
    );
    final averageAccuracy = progressList.isEmpty
        ? 0.0
        : progressList.map((p) => p.averageAccuracy).reduce((a, b) => a + b) /
            progressList.length;
    final totalHintsUsed = progressList.fold<int>(
      0,
      (sum, progress) =>
          sum +
          progress.lessonProgresses.fold<int>(
            0,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            ...progressList
                .take(5)
                .map((progress) => _buildAccuracyBar(progress)),
          ],
        ),
      ),
    );
